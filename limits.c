...
void limits_init()
{
	avr_var.LIMIT_DDR &=~ (LIMIT_MASK); // Set as input pins

	pio_configure(PIOA, PIO_INPUT, LIMIT_MASK, PIO_PULLUP);

	// default pull up is enable and there is no option to change it
	#ifdef DISABLE_LIMIT_PIN_PULL_UP
		avr_var.LIMIT_PORT &=~ (LIMIT_MASK); // Normal low operation. Requires external pull-down.
	#else
	// here default only option
		avr_var.LIMIT_PORT |= (LIMIT_MASK);  // Enable internal pull-up resistors. Normal high operation.
	#endif

	// ustwaiæ przerwania na okreœlonych pinach, w main jest przyka³d jak to zrobiæ
  if(bit_istrue(settings.flags, BITFLAG_HARD_LIMIT_ENABLE)){
    avr_var.LIMIT_PCMSK |= LIMIT_MASK; // Enable specific pins of the Pin Change Interrupt
    //PCICR |= (1 << LIMIT_INT); // Enable Pin Change Interrupt
	cfg_interrupt_pioa(LIMIT_MASK | CONTROL_MASK, 15);
  }else{
    pio_disable_interrupt(PIOA, LIMIT_MASK);
  };
}
...
void limits_disable()
{
	// wy³¹czyæ przerewania od LIMITS GPIO
...
uint32_t limits_get_state()
{
  uint32_t limit_state = 0;
  uint32_t pin = LIMIT_MASK; // (avr_var.LIMIT_PIN & LIMIT_MASK);
...
// limits need new hardware implementation due to different interrupt services routine mechanism.
  ...
  void LIMIT_INT_vect()
  {
	...
	void LIMIT_INT_vect()
	{
		puts("software debounce limit switch is native here, this option and following code doesn't make sense now");
	}
...
void limits_go_home(uint8_t cycle_mask)
{
  ...
  uint32_t n_cycle = (2*N_HOMING_LOCATE_CYCLE+1);
  uint32_t step_pin[N_AXIS];

  #ifdef ENABLE_DUAL_AXIS
    uint32_t step_pin_dual;
