...
uint32_t volatile actual_int_mask = 0;

uint32_t cfg_interrupt_pioa(uint32_t bitmask, uint32_t filter_time){
	// pins must be input type
	actual_int_mask |= (bitmask);

	pio_configure(PIOA, PIO_INPUT, actual_int_mask, PIO_PULLUP);
	// set debouncing filter
	REG_PIOA_IFER =	actual_int_mask;
	REG_PIOA_IFSCER = actual_int_mask;

	if(filter_time > 15000){
		return 0;
	}else{
		REG_PIOA_SCDR = filter_time;
	}
	// set interrupt
	REG_PIOA_AIMER = actual_int_mask;
	REG_PIOA_ESR = actual_int_mask;
	REG_PIOA_FELLSR = actual_int_mask;
	REG_PIOA_IER = actual_int_mask;
	return actual_int_mask;
}
...
uint32_t settings_read_coord_data(uint8_t coord_select, float *coord_data)
{
  uint32_t addr = coord_select*(sizeof(float)*N_AXIS+1) + EEPROM_ADDR_PARAMETERS;
...
uint8_t read_global_settings() {
  ...
  printf("read global settings\r\n");

  uint8_t version = eeprom_get_char(0);

  printf("version read %u \r\n", version);
...
void settings_init()
{
	...
	settings_t* rt = &defaults;

	for(uint8_t i=0; i<sizeof(settings_t); i++){
		here_is_your_eeprom_dude[i] = (uint8_t *)(&rt[i]);
	};

	here_is_your_eeprom_dude[0] = 10;
...
{
  if ( axis_idx == X_AXIS ) { return((X_STEP_BIT)); }
  if ( axis_idx == Y_AXIS ) { return((Y_STEP_BIT)); }
  if ( axis_idx == Z_AXIS ) { return((Z_STEP_BIT)); }
  return((C_STEP_BIT));
}
...
{
  if ( axis_idx == X_AXIS ) { return((X_DIRECTION_BIT)); }
  if ( axis_idx == Y_AXIS ) { return((Y_DIRECTION_BIT)); }
  if ( axis_idx == Z_AXIS ) { return((Z_DIRECTION_BIT)); }
  return((C_DIR_BIT));
}
