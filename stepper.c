...
typedef struct {
  ...
  uint32_t direction_bits;
    ...
    uint32_t is_pwm_rate_adjusted; // Tracks motions that require constant laser power/rate
  #endif
} st_block_t;
...
typedef struct {
  ...
  #else
    uint32_t prescaler;      // Without AMASS, a prescaler is required to adjust for slow timing.
  #endif
  #ifdef VARIABLE_SPINDLE
...
void st_wake_up()
{
  // Enable stepper drivers.
	if(bit_istrue(settings.flags,BITFLAG_INVERT_ST_ENABLE)){
		avr_var.STEPPERS_DISABLE_PORT |= (STEPPERS_DISABLE_BIT);

		pio_enable_output_write(PIOA, avr_var.STEPPERS_DISABLE_PORT);
		pio_sync_output_write(PIOA, avr_var.STEPPERS_DISABLE_PORT);
		pio_disable_output_write(PIOA, avr_var.STEPPERS_DISABLE_PORT);
...
void TIMER1_COMPA_vect(void)
{
	...
	avr_var.DIRECTION_PORT = (avr_var.DIRECTION_PORT & ~DIRECTION_MASK) | (st.dir_outbits & DIRECTION_MASK);

	pio_enable_output_write(PIOA, avr_var.DIRECTION_PORT);
	pio_sync_output_write(PIOA, avr_var.DIRECTION_PORT);
	pio_disable_output_write(PIOA, avr_var.DIRECTION_PORT);


	// Then pulse the stepping pins
	#ifdef STEP_PULSE_DELAY
		st.step_bits = (avr_var.STEP_PORT & ~STEP_MASK) | st.step_outbits; // Store out_bits to prevent overwriting.
	#else  // Normal operation
		avr_var.STEP_PORT = (avr_var.STEP_PORT & ~STEP_MASK) | (st.step_outbits & STEP_MASK);

		pio_enable_output_write(PIOA, avr_var.STEP_PORT);
		pio_sync_output_write(PIOA, avr_var.STEP_PORT);
		pio_disable_output_write(PIOA, avr_var.STEP_PORT);
	#endif
	...
	// now timer0 count from 0 (counter is reset on triger) to REG_TC0_RC0
	REG_TC0_RC0 = st.step_pulse_time; // Reload Timer0 counter
  ...
  if (st.exec_segment == NULL) {
    ...
    if (segment_buffer_head != segment_buffer_tail) {
      ...
      st.exec_segment = &segment_buffer[segment_buffer_tail];
        ...
        avr_var.TCCR1B = 3;//(avr_var.TCCR1B & ~(0x07<<CS10)) | (st.exec_segment->prescaler<<CS10);
		Timer2_set_freq(250);
      #endif

      // Initialize step segment timing per step and load number of steps to execute.
      //OCR1A = st.exec_segment->cycles_per_tick;
	  REG_TC1_RC0 = st.exec_segment->cycles_per_tick;
  ...
  if (st.counter_c > st.exec_block->step_event_count) {
    st.step_outbits |= (C_STEP_BIT);
    st.counter_c -= st.exec_block->step_event_count;
    if (st.exec_block->direction_bits & (C_DIR_BIT)) { sys_position[C_AXIS]--; }
...
void TIMER0_OVF_vect(void)
{
  ...
  avr_var.STEP_PORT = (avr_var.STEP_PORT & ~STEP_MASK) | (step_port_invert_mask & STEP_MASK);

  pio_enable_output_write(PIOA, avr_var.STEP_PORT);
  pio_sync_output_write(PIOA, avr_var.STEP_PORT);
  pio_disable_output_write(PIOA, avr_var.STEP_PORT);

  avr_var.TCCR0B = 0; // Disable Timer0 to prevent re-entering this interrupt when it's not needed.
  tc_stop(TC0, 0);
}
  ...
  {
    avr_var.STEP_PORT = st.step_bits & STEP_MASK; // Begin step pulse.
  ...
  void TIMER0_COMPA_vect()
  {
	  puts("  void TIMER0_COMPA_vect() not used");
  }
#endif
...
void st_reset()
{
  ...
  avr_var.DIRECTION_PORT = (avr_var.DIRECTION_PORT & ~DIRECTION_MASK) | (dir_port_invert_mask & DIRECTION_MASK);
...
void stepper_init()
{
  ...
  avr_var.STEP_DDR |= STEP_MASK;
  pio_configure(PIOA, PIO_OUTPUT_0, avr_var.STEP_DDR, PIO_PULLUP);

  avr_var.STEPPERS_DISABLE_DDR |= STEPPERS_DISABLE_BIT;
  pio_configure(PIOA, PIO_OUTPUT_0, avr_var.STEPPERS_DISABLE_DDR, PIO_PULLUP);

  avr_var.DIRECTION_DDR |= DIRECTION_MASK;
  pio_configure(PIOA, PIO_OUTPUT_0, avr_var.DIRECTION_DDR, PIO_PULLUP);


  // Configure Timer 1: Stepper Driver Interrupt
  //TCCR1B &= ~(1<<WGM13); // waveform generation = 0100 = CTC
  //clear timer on compare --> use reg_tc1_rc as ocr1a
  //TCCR1B |=  (1<<WGM12);
  ...
  tc_disable_interrupt(TC0, 0, TC_IER_CPAS | TC_IER_CPCS);


  avr_var.TCCR0A = 0; // Normal operation


  avr_var.TCCR0B = 0; // Disable Timer0 until needed
  tc_stop(TC0, 0);
  ...
  while (segment_buffer_tail != segment_next_head) { // Check if we need to fill the buffer.
      ...
      */
      ...
      if (st_prep_block->is_pwm_rate_adjusted || (sys.step_control & STEP_CONTROL_UPDATE_SPINDLE_PWM)) {
        if (pl_block->condition & (PL_COND_FLAG_SPINDLE_CW | PL_COND_FLAG_SPINDLE_CCW)) {
		  ...
		  serial_mode_normal();
		  printf("spindle_compute_pwm_value\r\n");
		  serial_mode_grbl();
        } else {
    ...
    */
    ...
    uint32_t cycles = ceil( (TICKS_PER_MICROSECOND*1000000*60)*inv_rate ); // (cycles/step)
      ...
      if (cycles < (1UL << 16)) { prep_segment->cycles_per_tick = cycles; } // < 65536 (4.1ms @ 16MHz) // now it is different
	  ...
	  if(cycles < 524288){ // < 524288 (3.49ms @ 150MHz
        prep_segment->prescaler = 1; // prescaler: 0 -> 8

		//prep_segment->cycles_per_tick = cycles;
		prep_segment->cycles_per_tick = cycles >> 3; // min divider is 8

      //} else if (cycles < (1UL << 19)) { // < 524288 (32.8ms@16MHz)
	} else if (cycles < 2097152){ // < 2097152 @ 150MHz/32 -> 13.98ms
        prep_segment->prescaler = 2; // prescaler: 8 -> 32

		//prep_segment->cycles_per_tick = cycles >> 3;
		prep_segment->cycles_per_tick = cycles >> 5; // /32

      } else {
        prep_segment->prescaler = 3; // prescaler: 64 -> 128
        //if (cycles < (1UL << 22)) { // < 4194304 (262ms@16MHz)
		if (cycles < 8388608) { // 8388608 @ 150MHz / 128 -> 55.92 ms
          //prep_segment->cycles_per_tick =  cycles >> 6;

		  prep_segment->cycles_per_tick =  cycles >> 7;	// /128
        } else { // Just set the slowest speed possible. (Around 4 step/sec.) ~17step.s for 150Hz
