...
#define GC_FAIL(status) return(status);
...
uint8_t gc_execute_line(char *line)
{
  ...
  while (line[char_counter] != 0) { // Loop until no more g-code words in line.
    ...
    if((letter < 'A') || (letter > 'Z')) { GC_FAIL(STATUS_EXPECTED_COMMAND_LETTER); } // [Expected word letter]
    char_counter++;
    if (!read_float(line, &char_counter, &value)) { GC_FAIL(STATUS_BAD_NUMBER_FORMAT); } // [Expected word value]
    ...
    switch(letter) {
        ...
        switch(int_value) {
          case 10: case 28: case 30: case 92:
            ...
            if (mantissa == 0) { // Ignore G28.1, G30.1, and G92.1
              if (axis_command) { GC_FAIL(STATUS_GCODE_AXIS_COMMAND_CONFLICT); } // [Axis word/command conflict]
          ...
          case 4: case 53:
            ...
            if ((int_value == 28) || (int_value == 30) || (int_value == 92)) {
              if (!((mantissa == 0) || (mantissa == 10))) { GC_FAIL(STATUS_GCODE_UNSUPPORTED_COMMAND); }
          ...
          case 80:
            ...
            if (int_value == 38){
              if (!((mantissa == 20) || (mantissa == 30) || (mantissa == 40) || (mantissa == 50))) {
                GC_FAIL(STATUS_GCODE_UNSUPPORTED_COMMAND); // [Unsupported G38.x command]
              }
          ...
          case 90: case 91:
            if (mantissa == 0) {
              ...
              if ((mantissa != 10) || (int_value == 90)) { GC_FAIL(STATUS_GCODE_UNSUPPORTED_COMMAND); } // [G90.1 not supported]
              mantissa = 0; // Set to zero to indicate valid non-integer G command.
          ...
          case 43: case 49:
            ...
            } else { GC_FAIL(STATUS_GCODE_UNSUPPORTED_COMMAND); } // [Unsupported G43.x command]
            mantissa = 0; // Set to zero to indicate valid non-integer G command.
            break;
          ...
          case 61:
            ...
            if (mantissa != 0) { GC_FAIL(STATUS_GCODE_UNSUPPORTED_COMMAND); } // [G61.1 not supported]
            // gc_block.modal.control = CONTROL_MODE_EXACT_PATH; // G61
            break;
          default: GC_FAIL(STATUS_GCODE_UNSUPPORTED_COMMAND); // [Unsupported G command]
        }
        if (mantissa > 0) { GC_FAIL(STATUS_GCODE_COMMAND_VALUE_NOT_INTEGER); } // [Unsupported or invalid Gxx.x command]
        // Check for more than one command per modal group violations in the current block
        // NOTE: Variable 'word_bit' is always assigned, if the command is valid.
        if ( bit_istrue(command_words,bit(word_bit)) ) { GC_FAIL(STATUS_GCODE_MODAL_GROUP_VIOLATION); }
      ...
      case 'M':
        ...
        if (mantissa > 0) { GC_FAIL(STATUS_GCODE_COMMAND_VALUE_NOT_INTEGER); } // [No Mxx.x commands]
        switch(int_value) {
          ...
          case 3: case 4: case 5:
            ...
            case 115:
              ...
              break;
          #endif
          default: GC_FAIL(STATUS_GCODE_UNSUPPORTED_COMMAND); // [Unsupported M command]
        }
      ...
      default:
        ...
        switch(letter){
          ...
          case 'T': word_bit = WORD_T;
					  if (value > MAX_TOOL_NUMBER) { GC_FAIL(STATUS_GCODE_MAX_VALUE_EXCEEDED); }
            gc_block.values.t = int_value;
						break;
          case 'X': word_bit = WORD_X; gc_block.values.xyz[X_AXIS] = value; axis_words |= (1<<X_AXIS); break;
          case 'Y': word_bit = WORD_Y; gc_block.values.xyz[Y_AXIS] = value; axis_words |= (1<<Y_AXIS); break;
          case 'Z': word_bit = WORD_Z; gc_block.values.xyz[Z_AXIS] = value; axis_words |= (1<<Z_AXIS); break;
          default: GC_FAIL(STATUS_GCODE_UNSUPPORTED_COMMAND);
        }

        // NOTE: Variable 'word_bit' is always assigned, if the non-command letter is valid.
        if (bit_istrue(value_words,bit(word_bit))) { GC_FAIL(STATUS_GCODE_WORD_REPEATED); } // [Word repeated]
        // Check for invalid negative values for words F, N, P, T, and S.
        // NOTE: Negative value check is done here simply for code-efficiency.
        if ( bit(word_bit) & (bit(WORD_F)|bit(WORD_N)|bit(WORD_P)|bit(WORD_T)|bit(WORD_S)) ) {
          if (value < 0.0) { GC_FAIL(STATUS_NEGATIVE_VALUE); } // [Word value cannot be negative]
        }
  ...
  */
  ...
  if (bit_istrue(value_words,bit(WORD_N))) {
    ...
    if (gc_block.values.n > MAX_LINE_NUMBER) { GC_FAIL(STATUS_GCODE_INVALID_LINE_NUMBER); } // [Exceeds max line number]
  }
  ...
  if (gc_parser_flags & GC_PARSER_JOG_MOTION) {
    if (bit_isfalse(value_words,bit(WORD_F))) { GC_FAIL(STATUS_GCODE_UNDEFINED_FEED_RATE); }
    ...
    if (gc_block.modal.feed_rate == FEED_RATE_MODE_INVERSE_TIME) { // = G93
      ...
      if (axis_command == AXIS_COMMAND_MOTION_MODE) {
        if ((gc_block.modal.motion != MOTION_MODE_NONE) && (gc_block.modal.motion != MOTION_MODE_SEEK)) {
          if (bit_isfalse(value_words,bit(WORD_F))) { GC_FAIL(STATUS_GCODE_UNDEFINED_FEED_RATE); } // [F word missing]
        }
      }
  ...
  if (gc_block.non_modal_command == NON_MODAL_DWELL) {
    if (bit_isfalse(value_words,bit(WORD_P))) { GC_FAIL(STATUS_GCODE_VALUE_WORD_MISSING); } // [P word missing]
    bit_false(value_words,bit(WORD_P));
  }
  ...
  if (axis_command == AXIS_COMMAND_TOOL_LENGTH_OFFSET ) { // Indicates called in block.
    if (gc_block.modal.tool_length == TOOL_LENGTH_OFFSET_ENABLE_DYNAMIC) {
      if (axis_words ^ (1<<TOOL_LENGTH_OFFSET_AXIS)) { GC_FAIL(STATUS_GCODE_G43_DYNAMIC_AXIS_ERROR); }
    }
  }
  ...
  if ( bit_istrue(command_words,bit(MODAL_GROUP_G12)) ) { // Check if called in block
    if (gc_block.modal.coord_select > N_COORDINATE_SYSTEM) { GC_FAIL(STATUS_GCODE_UNSUPPORTED_COORD_SYS); } // [Greater than N sys]
    if (gc_state.modal.coord_select != gc_block.modal.coord_select) {
      if (!(settings_read_coord_data(gc_block.modal.coord_select,block_coord_system))) { GC_FAIL(STATUS_SETTING_READ_FAIL); }
    }
  }
  ...
  switch (gc_block.non_modal_command) {
    case NON_MODAL_SET_COORDINATE_DATA:
      ...
      if (!axis_words) { GC_FAIL(STATUS_GCODE_NO_AXIS_WORDS) }; // [No axis words]
      if (bit_isfalse(value_words,((1<<WORD_P)|(1<<WORD_L)))) { GC_FAIL(STATUS_GCODE_VALUE_WORD_MISSING); } // [P/L word missing]
      coord_select = trunc(gc_block.values.p); // Convert p value to int.
      if (coord_select > N_COORDINATE_SYSTEM) { GC_FAIL(STATUS_GCODE_UNSUPPORTED_COORD_SYS); } // [Greater than N sys]
      if (gc_block.values.l != 20) {
        if (gc_block.values.l == 2) {
          if (bit_istrue(value_words,bit(WORD_R))) { GC_FAIL(STATUS_GCODE_UNSUPPORTED_COMMAND); } // [G10 L2 R not supported]
        } else { GC_FAIL(STATUS_GCODE_UNSUPPORTED_COMMAND); } // [Unsupported L]
      }
      ...
      if (!settings_read_coord_data(coord_select,gc_block.values.ijk)) { GC_FAIL(STATUS_SETTING_READ_FAIL); } // [EEPROM read fail]
      ...
      switch (gc_block.non_modal_command) {
        ...
        case NON_MODAL_GO_HOME_1: // G30
          ...
          if (gc_block.non_modal_command == NON_MODAL_GO_HOME_0) {
            if (!settings_read_coord_data(SETTING_INDEX_G28,gc_block.values.ijk)) { GC_FAIL(STATUS_SETTING_READ_FAIL); }
          } else { // == NON_MODAL_GO_HOME_1
            if (!settings_read_coord_data(SETTING_INDEX_G30,gc_block.values.ijk)) { GC_FAIL(STATUS_SETTING_READ_FAIL); }
          }
          if (axis_words) {
        ...
        case NON_MODAL_ABSOLUTE_OVERRIDE:
          ...
          if (!(gc_block.modal.motion == MOTION_MODE_SEEK || gc_block.modal.motion == MOTION_MODE_LINEAR)) {
            GC_FAIL(STATUS_GCODE_G53_INVALID_MOTION_MODE); // [G53 G0/1 not active]
          }
          break;
      }
  }
  ...
  if (gc_block.modal.motion == MOTION_MODE_NONE) {
    ...
    if (axis_words) { GC_FAIL(STATUS_GCODE_AXIS_WORDS_EXIST); } // [No axis words allowed]
  ...
  // was explicitly commanded in the g-code block.
    ...
    // the value must be positive. In inverse time mode, a positive value must be passed with each block.
      ...
      if (gc_block.values.f == 0.0) { GC_FAIL(STATUS_GCODE_UNDEFINED_FEED_RATE); } // [Feed rate undefined]
      ...
      switch (gc_block.modal.motion) {
        ...
        case MOTION_MODE_CCW_ARC:
          ...
          if (!axis_words) { GC_FAIL(STATUS_GCODE_NO_AXIS_WORDS); } // [No axis words]
          if (!(axis_words & (bit(axis_0)|bit(axis_1)))) { GC_FAIL(STATUS_GCODE_NO_AXIS_WORDS_IN_PLANE); } // [No axis words in plane]
          ...
          if (value_words & bit(WORD_R)) { // Arc Radius Mode
            ...
            if (isequal_position_vector(gc_state.position, gc_block.values.xyz)) { GC_FAIL(STATUS_GCODE_INVALID_TARGET); } // [Invalid target]
            ...
            */
            ...
            if (h_x2_div_d < 0) { GC_FAIL(STATUS_GCODE_ARC_RADIUS_ERROR); } // [Arc radius error]
            ...
            */
            ...
            if (!(ijk_words & (bit(axis_0)|bit(axis_1)))) { GC_FAIL(STATUS_GCODE_NO_OFFSETS_IN_PLANE); } // [No offsets in plane]
            ...
            if (delta_r > 0.005) {
              if (delta_r > 0.5) { GC_FAIL(STATUS_GCODE_INVALID_TARGET); } // [Arc definition error] > 0.5mm
              if (delta_r > (0.001*gc_block.values.r)) { GC_FAIL(STATUS_GCODE_INVALID_TARGET); } // [Arc definition error] > 0.005mm AND 0.1% radius
            }
          }
          break;
  ...
  if (value_words) { GC_FAIL(STATUS_GCODE_UNUSED_WORDS); } // [Unused words]
  ...
  */
  ...
  if (gc_parser_flags & GC_PARSER_JOG_MOTION) {
    ...
    if (command_words & ~(bit(MODAL_GROUP_G3) | bit(MODAL_GROUP_G6) | bit(MODAL_GROUP_G0)) ) { GC_FAIL(STATUS_INVALID_JOG_COMMAND) };
    if (!(gc_block.non_modal_command == NON_MODAL_ABSOLUTE_OVERRIDE || gc_block.non_modal_command == NON_MODAL_NO_ACTION)) { GC_FAIL(STATUS_INVALID_JOG_COMMAND); }
  ...
  if (gc_state.modal.program_flow) {
    ...
    if (gc_state.modal.program_flow == PROGRAM_FLOW_PAUSED) {
      ...
      if (sys.state != STATE_CHECK_MODE) {
        if (!(settings_read_coord_data(gc_state.modal.coord_select,gc_state.coord_system))) { GC_FAIL(STATUS_SETTING_READ_FAIL); }
